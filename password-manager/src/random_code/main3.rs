...
use firestore::*;
use tokio;  // Required for async runtime

#[tokio::main]  // Make sure to use the Tokio runtime for async operations
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    // Retrieve your project ID from an environment variable (static for this example)
    let project_id = "passwordmanager-c8b1f";
    ...
    match FirestoreDb::new(&project_id).await {
        Ok(db) => {
            println!("Firestore client initialized successfully.");
            // Your Firestore operations go here
        },
        Err(e) => {
            println!("Error initializing Firestore client: {:?}", e);
            return Err(Box::new(e)); // Box the FirestoreError to return it as Box<dyn std::error::Error>
        }
    }
