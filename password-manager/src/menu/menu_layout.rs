...
use crate::menu::menu_functions::generate_password::generate_random_password_wrapper;
...
pub async fn menu_function(
    ...
    loop {
        ...
        println!(
            ...
            "[6]".yellow(), "Generate a password".bold(), "(Generate a secure and random password)".dimmed(),
            "[7]".red(), "Logout".bold(), "(Go Back to the starting menu)".dimmed()
        );   
        print!("Enter choice: ");
        ...
        match choice.trim() {
            ...
            "6" => { // Generate a secure password                
                generate_random_password_wrapper();
            }
            "7" => {
